...
export default function App() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#fff6e0', overflow: 'hidden' }}>
      ...
      {/* ── Main ──────────────────────────────────────────────── */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, maxHeight: '100vh', transition: 'margin-left 0.28s ease', marginLeft: isLanding ? 0 : undefined }}
        className={`app-main ${isLanding ? 'full-width' : ''}`}>
        ...
        {/* Page content */}
        <main style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', background: isLanding ? 'transparent' : '#fff6e0' }}>
        ...
        {!isLanding && <VoiceTranslateWidget />}
      </div>
    </div>
  );
}
